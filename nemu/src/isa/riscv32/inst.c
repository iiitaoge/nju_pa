...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, 
         uint32_t t = s->pc + 4; // 防止rs1和rd是同一个寄存器
         s->dnpc = (src1 + imm) & -1;
         R(rd) = t);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if ((int32_t)src1 >= (int32_t)src2) s->dnpc = imm + s->pc;);  // 据说要用补码
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai    , I, 
    uint32_t shamt = SEXT(BITS(imm, 5, 0), 6);
    int is_shamt5_zero = shamt & (1 << 5); // 检查第五位是否为0, 1 << 5 为 0100000
    assert(!is_shamt5_zero); // 如果不为0就报错
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll     , R, 
    uint32_t shamt = SEXT(BITS(src2, 4, 0), 5);
    R(rd) = src1 << shamt;
  );
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli    , I, 
    uint32_t shamt = SEXT(BITS(imm, 4, 0), 5);
    R(rd) = src1 >> shamt;
  );
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra     , R, 
    uint32_t shamt = SEXT(BITS(src2, 4, 0), 5);
    int32_t src1_int = (int32_t)src1;
    R(rd) = src1_int >> shamt;
  );
...
int isa_exec_once(Decode *s) {
  s->isa.inst.val = inst_fetch(&s->snpc, 4);  // 这个函数取出了长度为 4 字节的指令
